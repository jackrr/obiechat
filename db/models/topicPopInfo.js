		...
		TopicPopInfo.find({}, function(err, tpis) {
			...
			_.each(tpis, function(tpi) {
				updateFuncs.push(function(callback) {
					Topic.findOneAndUpdate({ _id: tpi.topicID }, { $set: { popularity: getPop(tpi) }}, function(err, topic) {
						if (err) return callback(err);
						console.log(topic.get('popularity'));
						TopicPopInfo.findOneAndUpdate({ _id: tpi._id }, { $set: { lastPopularity: topic.get('popularity') }}, function(err, tpinfo) {
							if (err) return callback(err);
							console.log("topic info for", tpinfo.slug, " lastpop set to: ", tpinfo.lastPopularity);
							callback();
						});
					});
				});
